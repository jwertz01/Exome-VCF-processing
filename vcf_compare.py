...
def main():
    ...
    # in HTML table. Stored in counts dict in VcfFile object.
    ...
    for f_name in args.vcf_file_names:
        ...
        if len(reader.samples) > 1:
            ...
            v = VcfFile(
                ...
                next_rec=reader.next(), curr_qc_values={}
            )
    ...
    with open(args.out_stats, 'w') as out_f:
        ...
        print_dict_list(
            out_f, 'Counts Per File', counts_norm, counts_per_file_categs,
        ...
        for x in overall_counts_categs:
            if x in [
                ...
                'Conflicts multiple indel', 'Low QC', 'Predict HET'
            ]:
                out_f.write('</tr><tr>')
            out_f.write(
...
def update_qc_counts(qc_counts, vcf_file_obj, cat, scale_factor):
    ...
    for qc_cat in qc_counts:
        qc_val = vcf_file_obj.curr_qc_values[qc_cat]
...
def write_ad_ratio(
    ...
    for x in files_per_variant:
        for v in files_per_variant[x]:
            if v.has_curr_variant == 'high_qual':
                total_ad0 += v.curr_qc_values['ad0']
                total_ad1 += v.curr_qc_values['ad1']
    ad_ratio = float(total_ad0) / total_ad1

    if ad_ratio <= hom_alt_ratio:
        ad_ratio_categ = 'HOM-ALT'
    elif ad_ratio > het_ratio and ad_ratio < (1.0 / het_ratio):
        ad_ratio_categ = 'HET'
...
def write_conclusion(
    ...
    if (max_qd < borderline_qd):
        ...
        if len(common_gt) == 1 and (
            (common_gt[0] == '0/1' and ad_ratio_categ == 'HET') or
            (common_gt[0] == '1/1' and ad_ratio_categ == 'HOM-ALT')
        ):
            if ad_ratio_categ == 'HOM-ALT':
                overall_counts['Predict_HOM_ALT'] += 1
            elif ad_ratio_categ == 'HET':
                overall_counts['Predict HET'] += 1
            out_f.write(' Predict %s.' % ad_ratio_categ)
        else:
            overall_counts['No prediction'] += 1
